...
public class AttaquePhysique extends Attaque {
	...
	public AttaquePhysique(String nom, String type, int puissance, int precision, int nombreRepetition) {
		super(nom, type, puissance, precision, nombreRepetition);
	}
		...
		if(null == attaquant || null == victime) {
			...
			if(this.getRepetitionRestantes() == 0) {
				...
				if(this.getPrecision() < precision) {
					...
					double dammage = ( (((2*attaquant.getNiveau()/5)*this.getPuissance()*((double)attaquant.getAttaque()/(double)victime.getDefense()))/50)+2);
					double critical = 1;
	...
	public AttaquePhysique genererMemeAttaque(boolean generer) {
		if(!generer) {
			...
			return new AttaquePhysique(super.getNom(), super.getType(), super.getPuissance(), super.getPrecision(), super.getNombreRepetition());
		}
	}
}
